...
namespace GreenValleyAuctionsSystem
{
    public partial class EmployeeDashboard : System.Web.UI.Page
    {
        ...
        protected void grdvwServiceTable_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            //checks 
            if (e.CommandName == "view")
            {
                ...
                GridViewRow row = grdvwServiceTable.Rows[rowIndex];

                //places the customer information into session variables 
                Session["customerID"] = row.Cells[1].Text;
                Session["FirstName"] = row.Cells[2].Text;
                Session["LastName"] = row.Cells[3].Text;
                Session["serviceID"] = row.Cells[4].Text;
                Session["ServiceName"] = row.Cells[5].Text;
                Session["timeDate"] = row.Cells[6].Text;
                Session["status"] = row.Cells[7].Text;

                //redirects the user to the ticket details page
